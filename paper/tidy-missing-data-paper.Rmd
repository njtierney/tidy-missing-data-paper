...
Missing values are often ignored when plotting data - so much so that the data visualisation software, MANET, is an acronym for "Missings Are Now Equally Treated" [@Unwin1996]. However, plots can help to reveal the type of missing value patterns, even those of MCAR, MAR or even MNAR. Here we summarise how to systematically explore missing patterns visually, and define useful plots to make relative to the nabular data structure.

## Overviews {#overviews}

- How to visualise a binary matrix of missing vs not, and the dependencies between those binary variables as well.
- These overview plots are looking at the shadow matrix, and the summaries of each of the variables
- In this section we  describe which part of the nabular matrix we are going to display (overview - just binary parts), the summaries are summaries of the binary, the histogram and points are both the missings and the complete values.

The  first step is to get an overview of the extent of the missingness. Overview visualisations for variables and cases are provided with `gg_miss_var` and `gg_miss_case` (Figure \@ref(fig:gg-miss-case-var)), drawing attention to the amount of missings, and ordering by missingness. The "airquality" dataset (available in base R), is shown, and contains daily air quality measurements in New York, from May to September, 1973. We learn in figure \@ref("fig:gg-miss-case-var")A-B, that two variables contain missings and approximately one third of observations have one missing value. A tiny number of observations are missing on two variables. These summary plots compute proportions from the shadow matrix. The numerical statistics can also be reported, and are discussed in the next section. 
...
The shadow matrix allows both the variables and cases to be displayed using a heatmap style visualization, `vis_miss()` from \pkg{visdat} [@visdat]. This also provides numerical summaries of missingness in the legend, and for each column (Figure \@ref(fig:gg-miss-case-var)C-D). The user can also apply clustering to the rows, and arrange columns by missingness (Figure \@ref(fig:gg-miss-case-var)D). Similar visualizations are available in other missing data packages such as \pkg{VIM}, \pkg{mi}, \pkg{Amelia}, and \pkg{MissingDataGUI}. A key improvement is `vis_miss()` orients the visualization analogous to a regular data structure: variables form columns in the visualization and are named at the top, and each row is an observation. Using \pkg{ggplot2} makes the plot easily customizable. 

```{r airquality-upset, fig.height = 3, fig.width = 4, fig.cap = "The pattern of missingness in the airquality dataset shown in an upset plot. Only Ozone and Solar.R have missing values, and Ozone has the most missing values. There are 2 cases where both Solar.R and Ozone have missing values."}
# 
gg_miss_upset(airquality) 

```

The number of times certain variables go missing together can be visualized using an "upset plot" [@Conway2017]. An alternative to a Venn diagram, an upset plot shows the size and features of overlapping sets, but scales well with more variables. This can be applied to missing values, and an upset plot can be constructed from the shadow matrix, Figure \@ref(fig:airquality-upset) visualises overlapping amounts of missing data in the airquality data. The bottom right shows the combinations of missingness, the top panel shows the size of these combinations, and the bottom left shows missingness in each variable. This shows us clearly that Ozone has the most missing values, and there are 2 cases where values in both Solar.R and Ozone are missing together, illustrating similar points to \@ref(fig:gg-miss-case-var).
...
**Imputing values below the range**. To visualize the amount of missings in each variable, the data is transformed into _nabular_ form, then values are imputed below the range of data using `impute_below` (by default imputing 10% below the range). Figure \@ref(fig:impute-shift-histogram)A shows a histogram of Ozone, the number of missing values on its left. Ozone is shown in a different color by identifying the imputed values in _nabular_ data, by referring to the variable `Ozone` as `Ozone_NA` in a fill aesthetic in ggplot. The addition of the counts of missings in the histogram allows us to understand there are many missings, and that we have a skewed distribution of Ozone. 
...
The nabular structure of the data reveals the distribution of the missing values

```
To know where these values might have been located. tying this back to the nabular structure, and also the terminology of MCAR MAR, etc. (We would expect if data were MCAR for these points to be representatively thoughout the histogram)

- be sure to clear up vague language (it is important to...etc., )
- Missings are often ignored
```

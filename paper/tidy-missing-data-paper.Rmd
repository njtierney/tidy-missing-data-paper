...
The convention for representing missingness is a matrix, $R$, for data $y$ with $i$ rows and $j$ columns:
...
values that integrates into analysis pipelines. This means a data structure that is easily transported, and not a special class reliant on attributes that can be lost in analysis. The $R$ matrix, where 0 and 1
...
indicator values of missingness. The values 0 and 1 can
be confusing representations of missing values - does
0 indicate an absence of observation, or the presence of a missing value? 
The shadow matrix could be interactively linked to the data.
...
We propose a new form for tidy representation of missing data based on these ideas, based on this past research. Four features are added to the shadow matrix to facilitate analysis of missing data, illustrated in Figure \@ref(fig:shadow-matrix-progress)

(**note** - condense details below into points here.)

1. **Missing value labels**. A simple label to the shadow matrix clear, "NA" and "!NA" for missing and not missing, represented as 0 and 1.
...
1. Special missing values: Building on 1, the values in the shadow matrix can be "special" missing values, indicated as `NA_instrument`, where "instrument" is a very short label indicating an instrument error resulting in a missing values.

1. Coordinated names: Variables in the shadow matrix gain the same name as in the data, with the suffix "_NA".

1. Cohesiveness: Binding the shadow matrix column-wise to the original data creates a cohesive _nabular_ data format, so that it cannot be out of sync with the data. useful for visualization, summaries, and tracking imputed values.

(**note - change R matrix to something else**)

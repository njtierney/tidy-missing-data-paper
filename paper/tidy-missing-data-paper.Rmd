...
(**considering removing these examples**)

For flexibility, there are scoped variants for `replace_with_na`: `_all`, `_if`, and `_at`. This means `replace_with_na` works for **all** variables with `replace_with_na_all`, **at** selected variables with `replace_with_na_at`, and **if** variables meet some condition with `replace_with_na_if`. The syntax for these is slightly different to `replace_with_na`, but powerful:
...
The `impute_below` function imputes values below the minimum of the data, with some jitter (small values of random noise) to reduce overplotting, but has options to change the default amount it is shifted below, and the amount of jitter. 
...
One drawback to imputation functions in this form is imputed value locations are not tracked. This issue is resolved with \code{nabular} data covered in Section \@ref(verbs-track).

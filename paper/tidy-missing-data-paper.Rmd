...
Features of tidy data were formally discussed in 2014 [@Wickham2014], subsequently generating much discussion (@Donoho2017 and commentary, @r4ds), and tools for data analysis. Tidy data is defined as:
...
1. Missing value label: A simple label to the shadow matrix clear, "NA" and "!NA" for missing and not missing, represented as 0 and 1 (Figure \@ref(fig:shadow-matrix-progress) parts 2 and 3). 
...
Other statistical programs represent missingness values as a full-stop, `.`, and allow for recording special missing values as `.a` - `.z`. The special values from these languages break the rule of "keeping one thing in a cell", as they record both the value and the multivariate missingness state. Additionally, special missing values in other statistical programming languages are encoded at a lower level in the fundamental `NA` type, which is not possible in R.
...
A consistent short suffix "`_NA`" for each variable keeps names coordinated throughout analysis (Figure \@ref(fig:shadow-matrix-progress) part 4). The suffix is short and easy to remember in analysis or visualization. It also makes a clear distinction with `var_NA` being a random variable of the missingness of a variable, `var`. This subtle change is important as it shifts the focus from the value of a variable to its missingness state, making intent clear when performing analysis.
...
Common missing data operations can be considered verbs: scan, replace, add, shadow, impute, track, and flag. Missing values can be **scanned** to find possible missings not coded as `NA`. These values can then be **replaced** with `NA`. To facilitate exploration, summaries of missingness can be **added** as a summary column to the original data. Data values can be augmented with **shadow** values, helping explore missing data, as well as facilitating the process of **imputing**, and **tracking**. Finally, unusual or specially coded missing values can be **flagged**.
...
Three missingness clusters are revealed by visualizing missingness in the whole dataset, clustering and arranging the rows and columns of the data (Figure \@ref(fig:applic-vis-miss)).

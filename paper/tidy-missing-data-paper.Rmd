...
If a data structure is to be added to facilitate exploring missing data, it needs to be simple to understand and transport with a data analysis. A useful template is the $B$ matrix, where 0 and 1 indicate not missing and missing (respectively), as it is very common in missing data literature.  This matrix was used to explore missing values in
...
There are some limitations to the shadow matrix, however. Namely, the values 0 and 1 can be confusing representations of missing values - does 0 indicate an absence of observation, or the presence of a missing value? 
...
1. **Missing value labels**. Simple labels, "NA" and "!NA", for missing and not missing, clearly identifies missing values for analysis and plotting. This is an improvement over ($B$), which makes it unclear if 0 indicates the absence (0) of observation, or the presence (1) of a missing value. Missing value labels can be represented as "factor" types values, with text values "NA" and "!NA" and underlying number values 1 and 0, for missing and not missing (Figure \@ref(fig:shadow-matrix-progress) parts 1-3). 

(NOTE: principle: clarity of labelling - transparent what the matrix means, because these could equally be "missing" or "present" - for this. Anybody looking at these values)
...
(NOTE: change cohesiveness to connectedness)
...
Missing values are often ignored when plotting data, but plots can help to reveal the type of missing value patterns, even those of MCAR, MAR or even MNAR.
Here we summarise systematically exploring the missing patterns visually, and define useful plots to make relative to the nabular data structure, working from coarse to fine levels of granularity in understanding missingness.

- The acronym MANET [@Unwin1996] stands for "Missings Are Now Equally Treated".


## Overviews {#overviews}

The  first step is to get an overview of the extent of the missingness. <!--Rather than highlighting the amount of complete data,-->  Overview visualisations for variables and cases are provided with `gg_miss_var` and `gg_miss_case` (Figure \@ref(fig:gg-miss-case-var)). These  missing data visualizations should draw attention to the amount of missings, and ordering by missingness. The "airquality" dataset (available in base R), included in base R, containing daily air quality measurements in New York, from May to September, 1973, is shown. Two of the variables contain missings and about one third of observations have one missing value. A tiny number of observations are missing on two variables. These summary plots compute proportions from the shadow matrix. The numerical statistics can also be reported, and is discussed in the next section. 
...
The number of times certain variables go missing together can be visualized using an "upset plot" [@Conway2017]. An alternative to a Venn diagram, an upset plot shows the size and features of overlapping sets, but scales well with more variables. Figure \@ref(fig:airquality-upset) shows the airquality data, and illustrates  that only Ozone and Solar.R have missing values. The bottom right show the combinations of missingness, the bottom left showing missingness in each variable, and the top showing the combinations of missingness. The bottom left shows Ozone has the most missing values, and the top shows 2 cases where both Solar.R and Ozone go missing together. The upset plot is constructed from the shadow matrix.

...
To evaluate imputations they need to be tracked. This is achieved by first using `nabular` data, then imputing, and imputed values can then be referred to by their shadow variable, `_NA` (Figure \@ref(fig:track-impute-example)). The code below shows the track pattern, first using `nabular`, then imputing with `impute_lm`. `label_shadow` then adds a label to facilitate identifying missings:
